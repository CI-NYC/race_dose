library(tidyverse)
is_outlier <- function(dose) { # function to find upper outliers using Tukey's fence
...
data_met_3 |> filter(wk3.outlier == 1)
...
saveRDS(data_met_3, here::here("data/sensitivity/data_met_week3_outlier.rds"))
...
data_met_4 |> filter(wk4.outlier == 1)
